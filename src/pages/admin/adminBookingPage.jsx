                    ...
                    <table className="min-w-full bg-white border border-gray-300 rounded-lg shadow-md">
                        <thead className="bg-gray-200">
                            <tr>
                                ...
                                <th className="px-4 py-2 text-left">Staring Date</th>
                                <th className="px-4 py-2 text-left">Ending Date</th>
                                <th className="px-4 py-2 text-left">Total Amount</th>
                                <th className="px-4 py-2 text-left">Approval Staus</th>
                        ...
                        <tbody>
                            {orders.map((order)=>(
                                <tr key={order._id} className="border-t hover:bg-gray-100 cursor-pointer" onClick={()=>{
                                    ...
                                    <td className="px-4 py-2">{new Date(order.startingDate).toLocalDateString()}</td>
                                    <td className="px-4 py-2">{order.totalAmount.tofixed(2)}</td>
                                    <td className="px-4 py-2">{order.status}</td>
                                    <td className="px-4 py-2">{new Date(order.orderDate).toLocalString()}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            )}
            {
                modelOpened &&(
                    <div className="fixed top-0 left-0 w-full h-full bg-[#00000075 flex justify-center items-center">
                        ...
                        <IoMdCloseCircleOutline className="absolute tap-2 right-2 text-3xl cursor-pointer hover:text-red-600" onClick={()=>setModelOpened(false)}/>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <p><span className="font-semibold">Starting Date:</span>{new Date(activeOrder.startingDate).toLocalDateString()}</p>
                                <p><span className="font-semibold">Ending Date:</span>{new Date(activeOrder.endingDate).toLocalDateString}</p>
                                ...
                                <p><span className="font-semibold">Order Date:</span>{new Date(activeOrder.orderDate).toLocalDateString()}</p>
                            </div>

                            <div className="w-full flex justify-cener items-center my-5">
                                <button onClick={()=>{
